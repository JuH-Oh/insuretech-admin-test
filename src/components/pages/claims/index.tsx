...
import { TYPE_COLOR_MAP, STATUS_VARIANT_MAP } from '@/components/common/constants';
...
const ClaimsView: React.FC<ClaimsViewProps> = ({ claims, onNavigate, isLoading = false }) => {
  ...
  }, [claims, sourceFilter, typeFilter, statusFilter, search]);
    ...
    },
    {
      ...
      render: (row) => {
        ...
        if (row.status === '현장조사중' || row.status === '접수') {
          return (
            <Button variant="primary" size="sm" onClick={(e) => { e.stopPropagation(); onNavigate('/field'); }}>
              현장조사 배정
            </Button>
          );
        }
        if (row.status === '산정완료') {
          return (
            <Button variant="green" size="sm" onClick={(e) => { e.stopPropagation(); onNavigate('/approve'); }}>
              승인 요청
            </Button>
          );
        }
