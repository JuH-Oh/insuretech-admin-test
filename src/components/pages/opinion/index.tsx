...
import { STATUS_COLOR_MAP, TYPE_COLOR_MAP } from '@/components/pages/opinion/constants';
...
const OpinionView: React.FC<OpinionViewProps> = ({ opinionItems, onSend, isLoading = false }) => {
  ...
  const columns: Column<OpinionItem>[] = [
    ...
    },
    {
      ...
      width: '120px',
      render: (row) => {
        const color = TYPE_COLOR_MAP[row.type] || '#64748B';
        return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: '전체 의견서', value: opinionItems.length, cls: 'text-txt' },
          { label: '발송 완료', value: opinionItems.filter((o) => o.status === '발송완료').length, cls: 'text-green' },
          { label: '작성 중', value: opinionItems.filter((o) => o.status === '작성중').length, cls: 'text-amber' },
          { label: '이의신청', value: objections.length, cls: 'text-red' },
        ].map((item) => (
          <div key={item.label} className="bg-card rounded-card border border-border p-4 text-center">
            {isLoading
              ...
              : <div className={`text-[22px] font-bold ${item.cls}`}>{item.value}</div>
            }
      ...
      <div className="flex gap-1 bg-border-light rounded-block p-1 w-fit">
        ...
        >
          면책 의견서{!isLoading && ` (${opinions.length})`}
        </button>
        <button
        ...
        >
          이의신청{!isLoading && ` (${objections.length})`}
        </button>
      </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <DetailCard title="의견서 정보">
                <div className="space-y-2 text-[13px]">
                  ...
                  <div className="flex justify-between"><span className="text-secondary">상태</span><span className="font-semibold" style={{ color: STATUS_COLOR_MAP[selected.status] }}>{selected.status}</span></div>
                  {selected.recipient && (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(selected.status === '작성중' || selected.status === '검토완료') && (
                  <Button variant="green" size="sm" onClick={() => { onSend(selected.id); setSelected(null); }}>발송</Button>
                )}
