...
import { DataTable, Button, DetailCard, Skeleton } from '@/components/common';
import { STATUS_COLOR_MAP } from '@/components/pages/approve/constants';
...
interface ApproveViewProps {
  approveItems: ApproveItem[];
  onApprove: (claimId: string, action: 'approved' | 'rejected' | 'revision_requested') => void;
  isLoading?: boolean;
}

const ApproveView: React.FC<ApproveViewProps> = ({ approveItems, onApprove, isLoading = false }) => {
  ...
  const columns: Column<ApproveItem>[] = [
    ...
    },
    {
      ...
      width: '90px',
      render: (row) => {
        const color = STATUS_COLOR_MAP[row.status] || '#64748B';
        return (
      ...
      render: (row) => {
        ...
        return (
          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
            <Button variant="green" size="sm" onClick={() => onApprove(row.claimId, 'approved')}>승인</Button>
            <Button variant="danger" size="sm" onClick={() => onApprove(row.claimId, 'rejected')}>반려</Button>
            <Button variant="secondary" size="sm" onClick={() => onApprove(row.claimId, 'revision_requested')}>보완요청</Button>
          </div>
        );
      },
    },
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-card rounded-card border border-border p-4 text-center">
          {isLoading ? <Skeleton className="h-[26px] w-[40px] mx-auto mb-2" /> : <div className="text-[22px] font-bold text-amber">{pending.length}</div>}
          <div className="text-[11px] text-secondary mt-1">승인 대기</div>
        </div>
        <div className="bg-card rounded-card border border-border p-4 text-center">
          {isLoading ? <Skeleton className="h-[26px] w-[40px] mx-auto mb-2" /> : <div className="text-[22px] font-bold text-green">{approveItems.filter((a) => a.status === '승인').length}</div>}
          <div className="text-[11px] text-secondary mt-1">승인 완료</div>
        </div>
        <div className="bg-card rounded-card border border-border p-4 text-center">
          {isLoading ? <Skeleton className="h-[26px] w-[80px] mx-auto mb-2" /> : <div className="text-[22px] font-bold text-primary">{pending.reduce((sum, a) => sum + a.finalAmount, 0).toLocaleString()}원</div>}
      ...
      <div className="flex gap-1 bg-border-light rounded-block p-1 w-fit">
        ...
        >
          승인 대기{!isLoading && ` (${pending.length})`}
        </button>
        <button
        ...
        >
          승인 이력{!isLoading && ` (${history.length})`}
        </button>
      </div>
            ...
            <div className="p-5 space-y-4">
              ...
              {selected.status === '대기' && (
                <div className="flex gap-2">
                  <Button variant="green" size="sm" onClick={() => { onApprove(selected.claimId, 'approved'); setSelected(null); }}>승인</Button>
                  <Button variant="danger" size="sm" onClick={() => { onApprove(selected.claimId, 'rejected'); setSelected(null); }}>반려</Button>
                  <Button variant="primary" size="sm" onClick={() => { onApprove(selected.claimId, 'revision_requested'); setSelected(null); }}>보완요청</Button>
                </div>
              )}
