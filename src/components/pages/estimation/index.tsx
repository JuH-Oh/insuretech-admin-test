...
const EstimationView: React.FC<EstimationViewProps> = ({ estimationItems, isLoading = false }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: '전체 적산', value: estimationItems.length, cls: 'text-primary' },
          { label: '확정 완료', value: estimationItems.filter((e) => e.confirmed).length, cls: 'text-green' },
          { label: '미확정', value: estimationItems.filter((e) => !e.confirmed).length, cls: 'text-amber' },
          { label: '보정 건수', value: estimationItems.filter((e) => e.adjustedTotal && e.adjustedTotal !== e.aiTotal).length, cls: 'text-txt' },
        ].map((item) => (
      ...
      <DataTable title="AI 적산 결과" columns={columns} data={estimationItems} onRowClick={(row) => setSelected(row)} isLoading={isLoading} />
