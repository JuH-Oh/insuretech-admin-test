...
import { DataTable, StatusPill, Button, DetailCard, Skeleton } from '@/components/common';
import type { Column } from '@/components/common';
import type { Claim } from '@/types/claims';
import type { EstimationItem } from '@/types/estimation';

interface TypeCViewProps {
  claims: Claim[];
...
const TypeCView: React.FC<TypeCViewProps> = ({ claims, estimationItems, onNavigate, isLoading = false }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'TYPE C 총 건수', value: `${claims.length}건`, color: '#00854A' },
          { label: '보정 완료', value: `${claims.filter((c) => c.corrected).length}건`, color: undefined, cls: 'text-amber' },
          { label: '승인 대기', value: `${claims.filter((c) => c.status === '승인대기').length}건`, color: undefined, cls: 'text-primary' },
        ].map((item) => (
          <div key={item.label} className="bg-card rounded-card border border-border p-4 text-center">
            {isLoading
              ? <Skeleton className="h-[26px] w-[48px] mx-auto mb-2" />
              : <div className={`text-[22px] font-bold ${item.cls ?? ''}`} style={item.color ? { color: item.color } : {}}>{item.value}</div>
            }
          ...
          <div className="fixed top-0 right-0 bottom-0 w-[480px] bg-card border-l border-border z-50 overflow-y-auto shadow-lg">
            <div className="p-5 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="primary" size="sm" onClick={() => onNavigate('/estimation')}>AI 적산 확인</Button>
                <Button variant="secondary" size="sm">금액 보정</Button>
                <Button variant="green" size="sm" onClick={() => onNavigate('/approve')}>승인 요청</Button>
              </div>
            </div>
          </div>
        </>
