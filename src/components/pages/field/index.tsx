...
import { STATUS_COLOR_MAP } from '@/components/pages/field/constants';
...
const FieldView: React.FC<FieldViewProps> = ({ waitingClaims, completedClaims, isLoading = false }) => {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center justify-between max-w-[600px]">
          {['배정대기', '조사중', '보고서접수', '검토완료'].map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-[12px] font-bold" style={{ backgroundColor: STATUS_COLOR_MAP[step] }}>
                {i + 1}
              </div>
      ...
      <div className="flex gap-1 bg-border-light rounded-block p-1 w-fit">
        ...
        >
          조사 대기{!isLoading && ` (${waitingClaims.length})`}
        </button>
        <button
        ...
        >
          조사 완료{!isLoading && ` (${completedClaims.length})`}
        </button>
      </div>
              ...
              <DetailCard title="조사 정보">
                <div className="space-y-2 text-[13px]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold" style={{ color: STATUS_COLOR_MAP[selected.fieldStatus || '배정대기'] }}>
